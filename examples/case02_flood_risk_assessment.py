...
包含4个设计工况
1. 设计流量75%: Q=45 m^3/s
2. 设计流量100%: Q=60 m^3/s  
3. 设计流量125%: Q=75 m^3/s
4. 设计流量150%: Q=90 m^3/s
...
for i, scheme in enumerate(schemes, 1):
    ...
    print(f"   流量: {scheme['Q']} m^3/s ({scheme['ratio']*100:.0f}%设计流量)")
...
if len(successful_results) > 0:
    ...
    print(f"{'':12} {'(m^3/s)':<12} {'(m)':<12} {'(m)':<12} {'':<12} {'':<15}")
    print("-" * 85)
    ...
    if safe_results:
        ...
        print(f"   安全流量上限: {max_safe_Q} m^3/s超高>1.0m")
    else:
        print(f"   无完全安全工况")
    
    marginal_results = [r for r in successful_results if 0.5 < r['freeboard'] <= 1.0]
    if marginal_results:
        print(f"   中等风险流量: {min([r['Q'] for r in marginal_results])}-{max([r['Q'] for r in marginal_results])} m^3/s")
    ...
    try:
        ...
        for result in successful_results:
            ...
            ax4.text(0.1, y_pos, f"{name} (Q={Q:.0f} m^3/s): {risk}",
                    fontsize=11)
            y_pos -= 0.10
        ...
        plt.tight_layout()
        plt.savefig('./case02_flood_risk_assessment.png', dpi=150, bbox_inches='tight')
    ...
    print(f"\n1. 运行调度:")
    if safe_results:
        print(f"   - 正常运行: 流量<={max([r['Q'] for r in safe_results])} m^3/s")
        print(f"   - 监测水位保持超高>1.0m")
