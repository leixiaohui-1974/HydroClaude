...
支渠需水量：
  • 支渠1（上游）: 30 m³/s（灌溉2000公顷）
  • 支渠2（中游）: 40 m³/s（灌溉2000公顷）
  • 支渠3（下游）: 30 m³/s（灌溉1000公顷）
...
print(f"""
支渠取水口位置：
  • 支渠1: {x_branch1/1000:.1f}km (需水{Q_branch1:.0f}m³/s)
  • 支渠2: {x_branch2/1000:.1f}km (需水{Q_branch2:.0f}m³/s)
  • 支渠3: {x_branch3/1000:.1f}km (需水{Q_branch3:.0f}m³/s)
""")
...
print(f"\n  支渠1 (x={x_branch1/1000:.1f}km):")
print(f"    实际流量: {Q_actual2_b1:.2f} m³/s")
print(f"  支渠2 (x={x_branch2/1000:.1f}km):")
print(f"    实际流量: {Q_actual2_b2:.2f} m³/s")
print(f"  支渠3 (x={x_branch3/1000:.1f}km):")
print(f"    实际流量: {Q_actual2_b3:.2f} m³/s")
...
print(f"\n方案对比表:")
print(f"| 支渠 | 需水(m³/s) | 方案1实际 | 方案1误差 | 方案2实际 | 方案2误差 |")
print(f"|------|----------|---------|---------|---------|---------|")
...
plt.tight_layout()
plt.savefig('/workspace/case_irrigation_scheduling.png', dpi=150, bbox_inches='tight')
...
print(f"\n1. 配水方案选择:")
if avg_error1 < 5.0:
    print(f"   ✅ 方案1（均匀配水）可满足要求")
    print(f"      平均误差{avg_error1:.2f}% < 5%")
else:
    print(f"   ⚠️ 方案1误差较大({avg_error1:.2f}%)")
    print(f"   ✅ 推荐方案2（补偿配水）")
