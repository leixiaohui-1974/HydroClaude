...
# -*- coding: utf-8 -*-
"""
例子1扩展不同下游边界条件的影响研究 (HydrostaticCanalSolver高精度版本 - Refactored)
...
def main():
    """主函数"""
    print("=" * 80)
    print("例子1扩展不同下游边界条件的影响研究 (HydrostaticCanalSolver高精度版本)")
    print("=" * 80)
    ...
    # ========================================================================
    # 2. 第一阶段初始稳态Q=8.0 m^3/s
    # ========================================================================
    print("\n" + "=" * 80)
    print("第一阶段计算初始稳态 (Q=8.0 m^3/s)")
    print("=" * 80)
    ...
    for scenario_name, scenario_params in scenarios.items():
        ...
        result = solver.solve_steady_state(
            Q_target=Q_initial,
            h_downstream=scenario_params['h_downstream'],  # 注意这里用的是最终水深
            max_iterations=5000,
            convergence_tol = 0.1,
            dt=0.5,
            verbose=False
    ...
    # ========================================================================
    # 3. 第二阶段流量阶跃响应8.0 -> 10.0 m^3/s
    # ========================================================================
    print("\n" + "=" * 80)
    print("第二阶段流量阶跃响应 (8.0 -> 10.0 m^3/s)")
    print("=" * 80)
    ...
    for scenario_name, scenario_params in scenarios.items():
        ...
        results[scenario_name] = {
            ...
            'Q_final': solver.hu.copy(),
            'time': time_history,
            'h_upstream': h_upstream_history,
            'h_downstream': h_downstream_history,
            'Q_upstream': Q_upstream_history,
            'Q_downstream': Q_downstream_history,
    ...
    for scenario_name, data in results.items():
        ...
        axes[1].plot(time, data['h_downstream'], label=label,
                    color=params['color'], linewidth=2, alpha=0.8)
        axes[2].plot(time, data['Q_upstream'], label=label,
                    color=params['color'], linewidth=2, alpha=0.8)
    ...
    axes[0].axhline(h_uniform_initial, color='gray', linestyle=':', alpha=0.5,
                    label=f'Initial Theory ({h_uniform_initial:.4f}m)')
    axes[0].axhline(h_uniform_final, color='purple', linestyle=':', alpha=0.5,
    ...
    axes[1].set_ylabel('Downstream Depth (m)', fontsize=11)
    axes[1].set_title('System Response - Downstream Water Depth', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    fig_path1 = helper.get_output_path('04_boundary_timeseries_v2_refactored.png')
    ...
    ax1.set_title(f'Final Steady State - Water Depth (t={T_total:.0f}s)',
    ...
    plt.tight_layout()
    fig_path2 = helper.get_output_path('04_boundary_spatial_v2_refactored.png')
    plt.savefig(fig_path2, dpi=300, bbox_inches='tight')
    plt.close(fig2)
    print(f"   保存: {fig_path2.name}")
    ...
    for scenario_name, data in results.items():
        for i in range(len(data['time'])):
            timeseries_rows.append({
                'Scenario': data['params']['description'],
                'Time_s': data['time'][i],
                'h_upstream_m': data['h_upstream'][i],
                'h_downstream_m': data['h_downstream'][i],
                'Q_upstream_m3s': data['Q_upstream'][i],
                'Q_downstream_m3s': data['Q_downstream'][i]
            })

    timeseries_df = pd.DataFrame(timeseries_rows)
    table_path1 = helper.get_output_path('04_boundary_timeseries_v2_refactored.csv', subdir='tables')
    timeseries_df.to_csv(table_path1, index=False)
    print(f"   时间序列数据: {table_path1.name} ({len(timeseries_rows)} rows)")
    ...
    for scenario_name, data in results.items():
        for i in range(len(x)):
            spatial_rows.append({
                'Scenario': data['params']['description'],
                'Distance_m': x[i],
                'Water_Depth_m': data['h_final'][i],
                'Flow_Rate_m3s': data['Q_final'][i],
                'Flow_Error_pct': abs(data['Q_final'][i] - Q_final) / Q_final * 100
            })

    spatial_df = pd.DataFrame(spatial_rows)
    table_path2 = helper.get_output_path('04_boundary_spatial_v2_refactored.csv', subdir='tables')
    spatial_df.to_csv(table_path2, index=False)
    print(f"   空间分布数据: {table_path2.name} ({len(spatial_rows)} rows)")

    # 保存验证报告使用恒定均匀流场景
    print("  保存验证报告...")
    report_path = helper.get_output_path('04_boundary_validation_report_refactored.txt', subdir='reports')
    ...
    print(f"\n生成的文件:")
    print(f"  Figures:")
    print(f"    - {fig_path1.name} (时间序列)")
    print(f"    - {fig_path2.name} (空间分布)")
    print(f"  Tables:")
    print(f"    - {table_path1.name}")
    print(f"    - {table_path2.name}")
    print(f"  Reports:")
    ...
    print("  2. 高水位产生回水效应上游水深升高")
    print("  3. 低水位泵站导致落水曲线")
    print("  4. 流量守恒在所有场景下均达到优秀精度 (<0.01%)")

    print("\n 例子1扩展 (HydrostaticCanalSolver版 - Refactored) 运行成功")
    print("=" * 80)
