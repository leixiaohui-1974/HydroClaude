...
def run_optimized_example():
    ...
    fig_validation = scenario1_std['validator'].plot_flow_distribution(
        ...
        save_path=get_output_path('figures', '12_flow_validation_v2.png')
    )
    ...
    save_table(profile_data, '12_advanced_profile_v2.csv', index=False)

    # 保存验证报告
    print("  保存验证报告...")
    report_path = get_output_path('reports', '12_advanced_validation_report.txt')
    scenario1_std['validator'].save_report(report_path)

    print("\n生成的文件:")
    print("  Figures:")
    print("    - 12_advanced_optimized_comparison_v2.png (性能对比)")
    print("    - 12_flow_validation_v2.png (流量验证)")
    print("  Tables:")
    print(f"    - 12_advanced_optimized_comparison_v2.csv ({len(comparison_rows)} rows)")
    print(f"    - 12_advanced_profile_v2.csv ({len(x)} points)")
    print("  Reports:")
    print("    - 12_advanced_validation_report.txt")
