...
def run_sluice_gate_dynamics():
    ...
    # 闸门参数
    gate_opening = 5.0      # 闸门开度 (m) - 足够大以允许流量通过，但显示控制效果
    gate_Cd = 0.6           # 流量系数
 ...
 plt.close()
    ...
    table_path = helper.get_output_path('archive_01_sluice_gate_steady_profile_refactored.csv', subdir='tables')
    ...
    # GIF快照
    snapshot_interval = 100  # 每100步保存一次
    h_snapshots = []
    Q_snapshots = []
    t_snapshots = []
 ...
 plt.close()
    ...
    table_path = helper.get_output_path('archive_01_sluice_gate_time_series_refactored.csv', subdir='tables')
