...
# ==================== 初始条件 ====================
print("[2] 初始条件: 营养盐充足有藻类种群")
print("-" * 70)

# 水温: 夏季高温
T_init = np.full(n_cells, 28.0)  #  degC
print(f"水温: {T_init[0]:.1f}  degC (夏季高温)")
...
# 叶绿素: 初始有少量藻类
Chla_init = np.full(n_cells, 20.0)  # mug/L
...
print(f"日均气温: 32  degC (+/-6  degC)")
print(f"最大太阳辐射: 800 W/m^2")
print(f"白天时长: 14 小时")
...
print("[5] 时间积分")
print("-" * 70)
print(f"开始模拟 {n_days} 天高频输出捕捉昼夜变化...")
print()

# 输出存储每小时输出
output_interval = 2  # 每小时输出30分钟步长 x 2
output_times = []
output_T = []
output_DO = []
output_NH4 = []
...
for step in range(n_steps):
    ...
    # 每天打印一次
    if (step + 1) % (24 * 2) == 0:  # 每天
        print(f"Day {day:4.0f}: T={T.mean():5.1f} degC, DO={do_solver.DO.mean():6.2f}mg/L, "
              f"Chla={algae_solver.Chla.mean():7.1f}mug/L, "
...
print()
print("主要发现:")
print(f"  1. 藻华发展: {output_Chla[0]:.1f} -> {output_Chla[-1]:.1f} mug/L ({output_Chla[-1]/output_Chla[0]:.1f}倍)")
