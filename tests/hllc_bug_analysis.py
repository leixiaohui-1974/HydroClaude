...
代入：
    S_* = (0.5*g*h_R²*B - 0.5*g*h_L²*B + h_L*u_L*(S_L - u_L) - h_R*u_R*(S_R - u_R)) /
...
print("\n验证numerator是否等价...")
print("""
F_Q_L = Q_L² / (h_L * B) + 0.5 * g * h_L² * B
      = h_L * B * u_L² + 0.5 * g * h_L² * B

F_Q_R = Q_R² / (h_R * B) + 0.5 * g * h_R² * B
      = h_R * B * u_R² + 0.5 * g * h_R² * B

当前numerator = F_Q_R - F_Q_L + S_L * Q_L - S_R * Q_R
              = [h_R*B*u_R² + 0.5*g*h_R²*B] - [h_L*B*u_L² + 0.5*g*h_L²*B]
                + S_L*h_L*B*u_L - S_R*h_R*B*u_R

Toro numerator = (0.5*g*h_R²*B - 0.5*g*h_L²*B)
                 + h_L*u_L*(S_L - u_L) - h_R*u_R*(S_R - u_R)
               = 0.5*g*(h_R² - h_L²)*B
                 + h_L*u_L*S_L - h_L*u_L² - h_R*u_R*S_R + h_R*u_R²

对比:
当前: [h_R*B*u_R² - h_L*B*u_L²] + 0.5*g*(h_R² - h_L²)*B + [S_L*h_L*B*u_L - S_R*h_R*B*u_R]
Toro: [h_R*u_R² - h_L*u_L²] + 0.5*g*(h_R² - h_L²)*B + [S_L*h_L*u_L - S_R*h_R*u_R]

差异: 当前多了宽度B在速度平方项上！
❌ Bug发现: h_R*B*u_R² 应该是 h_R*u_R² (在Toro公式中是单位宽度)
但在代码中F_Q已经是总通量，所以公式应该是正确的...
...
print("""
潜在问题:
1. 当 S_L ≈ S_star 时，h_L_star → ∞ (分母接近零)
2. 当 S_L < S_star 时，h_L_star 变负 (违反物理意义)
3. 没有正定性检查
...
但是，这只检查了S_star计算的分母，没有检查h_star计算！
...
可能原因:
1. HLLC对静态问题更敏感，放大了Well-Balanced重构误差
2. 接触波分辨在u≈0时引入数值振荡
...
Fix 4: 改进Well-Balanced重构（长期）
------
当前Well-Balanced scheme可能引入O(dx²)误差
需要更高精度的hydrostatic reconstruction

Fix 5: 使用Exact Riemann Solver（终极方案）
------
对于Lake at Rest，只有Exact solver能达到机器精度
HLLC仍然是近似求解器
""")
...
print("="*80)
print("""
HLLC实现基本正确，但存在数值稳定性问题：
1. ✅ S_star公式正确
2. ✅ h_star公式正确
3. ❌ 缺少正定性检查
4. ❌ 缺少S_star范围检查
5. ⚠️ HLLC在Lake at Rest上表现差是"特性"不是bug
