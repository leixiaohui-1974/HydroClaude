...
def test_flux_comparison():
    ...
    print(f"  F_h = {F_h_hll:.6f} m³/s")
    print(f"  F_Q = {F_Q_hll:.6f} m³/s²")
    print()
    ...
    try:
        ...
        print(f"  F_h = {F_h_exact:.6f} m³/s")
        print(f"  F_Q = {F_Q_exact:.6f} m³/s²")
        print()

        # 对比
        print("差异:")
        print(f"  ΔF_h = {abs(F_h_exact - F_h_hll):.6f} m³/s ({abs(F_h_exact - F_h_hll)/abs(F_h_hll)*100:.2f}%)")
        print(f"  ΔF_Q = {abs(F_Q_exact - F_Q_hll):.6f} m³/s² ({abs(F_Q_exact - F_Q_hll)/abs(F_Q_hll)*100:.2f}%)")
        print()

        # 检查通量是否合理
        if abs(F_h_exact) > 100 or abs(F_Q_exact) > 10000:
            print(f"⚠️  警告: 精确求解器通量异常大!")
            print(f"  F_h_exact = {F_h_exact:.3e}")
            print(f"  F_Q_exact = {F_Q_exact:.3e}")
            return False

        if abs(F_h_exact - F_h_hll) / abs(F_h_hll) > 0.5:
            print("⚠️  警告: 精确与HLL通量差异>50%")
            return False
