...
测试场景:
1. 水静止在变化的底床上 (Q=0, /x=0)
2. 理论应该永远保持静止 (Q=0, h不变)
3. 数值如果不是well-balanced会产生spurious flow
...
def test_water_at_rest_sloped_bottom():
    """测试2: 斜底上的静水关键测试"""
    print("=" * 80)
    print("测试2: 斜底上的静水Well-Balanced关键测试")
    print("=" * 80)
    ...
    for well_balanced in [False, True]:
        ...
        # 初始条件水面平齐
        h_init = eta_surface - solver.z_b  # h =  - z_b
        ...
        if well_balanced:
            ...
            if max_Q < 1e-10 and eta_deviation < 1e-10:
                ...
                print(f"      Well-Balanced效果不理想")
        else:
            # 标准格式会产生spurious flow
            if max_Q > 1e-6:
                print(f"      产生spurious flow (符合预期)")
...
def test_water_at_rest_varying_slope():
    """测试3: 变化坡度上的静水最严格测试"""
    print("=" * 80)
    print("测试3: 变化坡度上的静水Well-Balanced最严格测试")
    print("=" * 80)
    ...
    # S0 = 0.01 + 0.005*sin(2pix/L) 正弦变化坡度
    ...
    for well_balanced in [False, True]:
        ...
        if well_balanced:
            ...
            if max_Q > 1e-5:
                print(f"      严重spurious flow (预期)")

    print()


def test_small_perturbation():
    """测试4: 小扰动传播well-balanced不影响动力学"""
    print("=" * 80)
    print("测试4: 小扰动传播验证well-balanced不影响正常动力学")
    print("=" * 80)
    ...
    for well_balanced in [False, True]:
        ...
        print(f"  500步后:")
        print(f"    max|Q|: {results[well_balanced]['max_Q']:.4f} m^3/s")
        print(f"    扰动已传播")
