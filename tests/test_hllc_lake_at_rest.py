...
def test_lake_at_rest_hll_vs_hllc():
    ...
    print(f"  η deviation: {eta_max_dev_init_hll:.2e} m (should be ~0)")
    ...
    print(f"  Max η deviation: {eta_max_dev_hll:.4f} m")
    print(f"  Mean η deviation: {eta_mean_dev_hll:.4f} m")
    print(f"  Max |Q|: {np.max(np.abs(solver_hll.Q)):.4e} m³/s")
    ...
    print(f"Initial state:")
    print(f"  η deviation: {eta_max_dev_init_hllc:.2e} m (should be ~0)")
    ...
    print(f"  Max η deviation: {eta_max_dev_hllc:.4f} m")
    print(f"  Mean η deviation: {eta_mean_dev_hllc:.4f} m")
    print(f"  Max |Q|: {np.max(np.abs(solver_hllc.Q)):.4e} m³/s")
    ...
    print(f"  {'Solver':<10} {'Max η dev (m)':<20} {'Mean η dev (m)':<20}")
    print(f"  {'-'*50}")
    ...
    if eta_max_dev_hllc < eta_max_dev_hll:
        ...
        print(f"\n⚠️  HLLC and HLL show similar dissipation (< 1% difference)")
    ...
    # 目标检查
    target = 0.1  # 10cm目标
    print(f"\nTarget: Max η deviation < {target}m")
...
def test_lake_at_rest_hllc_long_time():
    ...
    for t_check in check_times:
        ...
        print(f"\nt = {solver.t:6.1f}s: Max η dev = {eta_max_dev:.4f}m, Max |Q| = {results[-1]['max_Q']:.2e} m³/s")
...
if __name__ == '__main__':
    ...
    if success1 and success2:
        ...
        print("\n⚠️  Short-time test passed, but long-time stability issues")
