...
def test_supercritical_inlet():
    ...
    bc_right = {'type': 'h', 'value': 2.0}  # 下游缓流
    ...
    Fr_error = abs(Fr_final - Fr_upstream) / Fr_upstream * 100
    ...
    if h_error < tolerance and Q_error < tolerance and Fr_final > 0.9:
        ...
        if Q_error >= tolerance:
            print(f"   流量误差过大: {Q_error:.2f}% >= {tolerance}%")
        if Fr_final <= 0.9:
            print(f"   Froude数偏离急流状态: Fr={Fr_final:.3f} < 1.0")
        return False
