#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
测试使用'critical'边界条件类型
"""

import sys
import warnings
warnings.filterwarnings("ignore")
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))

import numpy as np
try:
    from solvers.godunov_fvm_solver import GodunvFVMSolver
except ImportError as e:
    print(f"Import error: {e}")
    print("Make sure project root is in sys.path")
    sys.exit(1)



def test_critical_bc():
    """使用critical边界条件"""

    print("\n" + "="*80)
    print("测试'critical'边界条件")
    print("="*80)

    B = 1.0
    n = 0.03
    S0 = 0.002
    L = 1000.0
    Q_bc = 2.0
    g = 9.81

    h_c = (Q_bc**2 / (g * B**2))**(1/3)

    print(f"\n参数：")
    print(f"  Q_left = {Q_bc} m^3/s")
    print(f"  边界条件类型: 'critical' (而不是'h')")

    # 创建求解器
    n_cells = 50
    solver = GodunvFVMSolver(
        width=B,
        length=L,
        n_cells=n_cells,
        manning_n=n,
        slope=S0,
        cfl=0.4,
        order=1,
        use_numba=False
    )

    # 初始化
    h_init = np.ones(n_cells) * h_c * 1.5
    Q_init = np.ones(n_cells) * Q_bc

    bc_left = {'type': 'Q', 'value': Q_bc}
    bc_right = {'type': 'critical'}  # 使用critical边界条件

    solver.initialize(h_init, Q_init, bc_left, bc_right)

    mass_initial = solver.initial_mass

    # 运行到稳态
    t_end = 50.0
    cumulative_inflow = 0.0
    cumulative_outflow = 0.0

    print(f"\n运行到 t={t_end}s...")

    step_count = 0
    while solver.t < t_end:
        dt = solver.compute_dt()
        solver.step()
        step_count += 1

        # 累积通量
        if solver.last_F_h is not None:
            F_left = solver.last_F_h[0]
            F_right = solver.last_F_h[-1]
            cumulative_inflow += F_left * dt * B
            cumulative_outflow += F_right * dt * B

    # 最终分析
    mass_final = np.sum(solver.h * solver.dx * B)
    delta_mass_actual = mass_final - mass_initial
    delta_mass_theory = cumulative_inflow - cumulative_outflow

    # 质量误差
    mass_error_pct = abs(mass_final - mass_initial) / mass_initial * 100

    # 通量守恒
    discrepancy_pct = abs(delta_mass_actual - delta_mass_theory) / abs(delta_mass_theory) * 100 if delta_mass_theory != 0 else 0

    # 流入流出比
    inflow_outflow_ratio = cumulative_inflow / cumulative_outflow if cumulative_outflow > 0 else 0

    print(f"\n结果：")
    print(f"  总步数：{step_count}")
    print(f"  质量误差：{mass_error_pct:.2f}%")
    print(f"  通量守恒：{discrepancy_pct:.2f}%")
    print(f"  累积流入：{cumulative_inflow:.2f} m^3")
    print(f"  累积流出：{cumulative_outflow:.2f} m^3")
    print(f"  流入/流出比：{inflow_outflow_ratio:.2f}")

    # 最终边界状态
    if solver.h[-1] > solver.eps_dry:
        u = solver.Q[-1] / (solver.h[-1] * B)
        Fr = abs(u) / np.sqrt(g * solver.h[-1])
        print(f"\n最终边界状态：")
        print(f"  h[-1] = {solver.h[-1]:.6f} m")
        print(f"  Q[-1] = {solver.Q[-1]:.6f} m^3/s")
        print(f"  Fr[-1] = {Fr:.6f}")

    print(f"\n判断：")
    if mass_error_pct < 5.0:
        print(f"   质量守恒良好 ({mass_error_pct:.2f}%)")
    else:
        print(f"   质量误差过大 ({mass_error_pct:.2f}%)")

    if 0.9 < inflow_outflow_ratio < 1.1:
        print(f"   流量平衡 (比值={inflow_outflow_ratio:.2f})")
    else:
        print(f"   流量不平衡 (比值={inflow_outflow_ratio:.2f})")

    # 对比h边界条件
    print(f"\n对比：")
    print(f"  - 使用'h'边界条件：质量误差40.67%, 流入/流出比1.86")
    print(f"  - 使用'critical'边界条件：质量误差{mass_error_pct:.2f}%, 流入/流出比{inflow_outflow_ratio:.2f}")

    print("\n" + "="*80)

    return {'mass_error': mass_error_pct, 'ratio': inflow_outflow_ratio}


if __name__ == "__main__":
    result = test_critical_bc()
