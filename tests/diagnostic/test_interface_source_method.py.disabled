...
# -*- coding: utf-8 -*-
"""
Interface Source Term Method测试

对比standard vs interface源项方法对MacDonald场景质量守恒的影响

目标：验证interface方法（Zhou's Surface Gradient Method启发）
     能否将质量误差从60%降低到<5%
"""
...
def test_source_method_comparison():
    """对比standard vs interface源项方法"""

    print("\n" + "="*80)
    print("Interface Source Term Method对比测试")
    print("="*80)
    ...
    print(f"\n预期：interface方法应显著改善质量守恒（目标：60% → <5%）")

    # 测试1: Standard方法（当前）
    print(f"\n{'='*80}")
    print("测试1: source_term_method='standard' (点值法)")
    print("="*80)
    ...
    solver1 = GodunvFVMSolver(
        ...
        order=1,
        well_balanced=False,
        source_term_method='standard',  # 标准点值法
        use_numba=False
    )
    ...
    # 测试2: Interface方法（新）
    print(f"\n{'='*80}")
    print("测试2: source_term_method='interface' (界面法 - Zhou启发)")
    print("="*80)
    ...
    solver2 = GodunvFVMSolver(
        ...
        order=1,
        well_balanced=False,
        source_term_method='interface',  # 界面法（新方法）
        use_numba=False
    )
    ...
    print(f"\n{'方法':<25} {'质量误差':>12} {'流量误差':>12} {'改善':>12}")
    print("-" * 65)
    print(f"{'standard (点值法)':<25} {mass_error_1:11.2f}% {abs(Q_avg_1-Q_bc)/Q_bc*100:11.2f}%")
    print(f"{'interface (界面法)':<25} {mass_error_2:11.2f}% {abs(Q_avg_2-Q_bc)/Q_bc*100:11.2f}%")
    ...
    if mass_error_2 < 5.0:
        if abs(mass_error_2) < abs(mass_error_1) * 0.1:
            print(f"  ✅ Interface方法显著改善质量守恒！ (误差<5%，改善>{90}%)")
        else:
            print(f"  ✅ Interface方法达到目标质量守恒 (误差<5%)")
    elif abs(mass_error_2) < abs(mass_error_1) * 0.5:
        print(f"  ⚠️ Interface方法显著改善但未达标 (改善>{50}%，但误差仍>{5}%)")
    elif abs(mass_error_2) < abs(mass_error_1):
        print(f"  ⚠️ Interface方法略有改善 (改善<{50}%)")
    else:
        print(f"  ✗ Interface方法无改善或变差")

    # 流量守恒评估
    if abs(Q_avg_2 - Q_bc) / Q_bc < 0.05:
        print(f"  ✅ Interface方法流量守恒良好 (<5%)")
    elif abs(Q_avg_2 - Q_bc) < abs(Q_avg_1 - Q_bc) * 0.5:
        print(f"  ⚠️ Interface方法流量有改善 (改善>50%)")
    elif abs(Q_avg_2 - Q_bc) < abs(Q_avg_1 - Q_bc):
        print(f"  ⚠️ Interface方法流量略有改善")
    else:
        print(f"  ✗ Interface方法流量无改善或变差")
    ...
    print("\nStandard方法（点值法）:")
    print("  S = g*A*(S0 - Sf)")
    print("  问题：源项在单元中心计算，与界面通量不平衡")
    print("\nInterface方法（界面法）:")
    print("  S_bed = -g*h*B*(z_b[i+1/2] - z_b[i-1/2])/dx")
    print("  S_friction = -g*A*Sf (点值)")
    print("  优点：底坡源项从界面值计算，与通量更好平衡（Zhou 2001）")
