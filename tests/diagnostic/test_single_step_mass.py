...
def test_single_step_mass():
    ...
    print(f"\n步后质量 = {mass_after:.3f} m³")
    print(f"  h = {solver.h}")
    print(f"\n质量变化 = {delta_mass:.3f} m³")
    ...
    if solver.last_F_h is not None:
        ...
        print(f"\n边界通量：")
        print(f"  F_left = {F_left:.3f} m²/s")
        print(f"  F_right = {F_right:.3f} m²/s")
        print(f"  净通量 = {net_flux:.3f} m²/s")
        print(f"\n理论质量变化 = {delta_mass_theory:.3f} m³")
        ...
        print(f"\n{'='*80}")
        print(f"实际 vs 理论：")
        print(f"  实际质量变化：{delta_mass:.3f} m³")
        print(f"  理论质量变化：{delta_mass_theory:.3f} m³")
        print(f"  差异：{error:.3f} m³ ({error_percent:.1f}%)")
        ...
        if error_percent < 1.0:
            ...
            # 检查每个单元的质量变化
            print(f"\n单元级质量变化：")
            print(f"  {'单元':<6} {'h_before':<10} {'h_after':<10} {'Δh':<10} {'Δm (m³)':<12}")
            print("-" * 60)
