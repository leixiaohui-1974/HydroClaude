...
echo -e "${BLUE}================================${NC}"
echo -e "${BLUE}浏览器测试环境搭建${NC}"
echo -e "${BLUE}================================${NC}\n"

# 1. 安装Playwright（推荐）
echo -e "${YELLOW}[1/4] 安装 Playwright...${NC}"
python3 -m pip install -q playwright pytest-playwright
...
# 2. 验证安装
echo -e "${YELLOW}[2/4] 验证安装...${NC}"
python3 -c "from playwright.sync_api import sync_playwright; print('✅ Playwright 可用')"

# 3. 创建测试脚本
echo -e "\n${YELLOW}[3/4] 创建测试脚本...${NC}"

cat > /workspace/web/browser_test.py << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x /workspace/web/browser_test.py

echo -e "${GREEN}✅ 测试脚本创建完成${NC}\n"

# 4. 显示使用说明
echo -e "${YELLOW}[4/4] 使用说明${NC}"
echo -e "${GREEN}================================${NC}"
echo -e "${GREEN}环境搭建完成！${NC}"
echo -e "${GREEN}================================${NC}\n"

echo -e "${BLUE}📖 运行测试:${NC}"
echo -e "   1. 启动服务: ${GREEN}./start_servers.sh${NC}"
echo -e "   2. 运行测试: ${GREEN}python3 browser_test.py${NC}\n"

echo -e "${BLUE}🔧 可用工具:${NC}"
echo -e "   - Playwright (浏览器自动化)"
echo -e "   - Chromium (无头浏览器)\n"

echo -e "${BLUE}📋 测试文件:${NC}"
echo -e "   - browser_test.py (自动化测试脚本)"
echo -e "   - test_screenshot.png (测试截图)"
echo -e "   - browser_test_report.json (测试报告)\n"
