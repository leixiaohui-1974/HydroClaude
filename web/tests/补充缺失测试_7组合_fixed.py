...
"""
补充缺失的7个组合测试 - 修复版
Supplement Missing Combination Tests - Fixed Version
...
def test_reservoir_channel_gate_irrigation():
    ...
    Q_gate, regime = gate.compute_discharge(h_upstream, h_channel)
    print(f"  闸门流量: {Q_gate:.2f}m³/s ({regime.value})")
    ...
    for hour in range(0, 13, 4):
        ...
        new_elev, _ = reservoir.route(Q_out, 0, dt)
...
def test_river_pump_weir_regulation():
    ...
    river = Channel(
        ...
        length=10000.0,
        shape=CrossSectionShape.NATURAL,
        width=30.0,
        side_slope=3.0,
        manning_n=0.035,
        slope=0.0001,
        start_position=0.0,
        end_position=10000.0
    )
    print(f"  河道: {river.name}")
    print(f"  河宽: {river.width}m")
    ...
    pump_station = PumpStation(
        name="Drainage-Pump",
        position=5000.0,
        pump_type=PumpType.PARALLEL,  # 并联运行
        num_pumps=3,
    ...
    # 给定扬程，计算流量（考虑3台并联）
    H_required = 25.0  # 扬程m
    Q_single = pump_station.compute_flow(H_required)
    Q_pump = Q_single * pump_station.num_pumps  # 3台并联
    print(f"  泵站排水: {Q_pump:.2f}m³/s (3台并联, 单台{Q_single:.2f}m³/s)")
    
    # 堰顶溢流
    h_total = h_river  # 总水深
    Q_weir = weir.compute_discharge(h_total, 0)
    print(f"  上游水深: {h_total:.2f}m")
    print(f"  溢流流量: {Q_weir:.2f}m³/s")
    
    # 水量平衡
    Q_total_out = Q_pump + Q_weir
    print(f"  总出流: {Q_total_out:.2f}m³/s")
    print(f"  水量平衡: {'基本平衡' if abs(Q_flood - Q_total_out)/Q_flood < 0.2 else '不平衡'}")
...
def test_canal_culvert_gate_drainage():
    ...
    from web.backend.core.structures.channel import Channel, ChannelType, CrossSectionShape
    from web.backend.core.structures.culvert import Culvert, CulvertType
    from web.backend.core.structures.advanced_gates import SluiceGate
    ...
    print(f"  涵洞流量: {Q_culvert:.2f}m³/s ({flow_type.value})")
    
    # 闸门下泄
    Q_gate, regime = gate.compute_discharge(h_canal, 0.5)
    print(f"  闸门流量: {Q_gate:.2f}m³/s ({regime.value})")
...
def test_pipe_valve_pump_pressure():
    ...
    # 泵站扬程
    Q_per_pump = Q / pump.num_pumps  # 2台并联，每台流量
    H_pump = pump.compute_head(Q_per_pump)
    print(f"\n泵站扬程: {H_pump:.2f}m (单台{Q_per_pump:.2f}m³/s)")
...
def test_reservoir_canal_sideweir_diversion():
    ...
    side_weir = SideWeir(
        ...
        position=5000.0,
        length=30.0,
        crest_height=2.0  # 修正：crest_height而不是crest_elevation
    )
    print(f"  侧堰: {side_weir.name}")
    print(f"  长度: {side_weir.length}m")
    print(f"  堰顶高程: {side_weir.crest_height}m")
    ...
    # 侧堰分流
    Q_divert, Q_downstream = side_weir.compute_discharge(h_main, Q_main, main_canal.width)
    ...
    for h in [2.5, 3.0, 3.5, 4.0]:
        Q_div, Q_down = side_weir.compute_discharge(h, Q_main, main_canal.width)
        ratio = Q_div / Q_main * 100 if Q_main > 0 else 0
        print(f"  水深={h:.1f}m: 分流={Q_div:.2f}m³/s, 下游={Q_down:.2f}m³/s ({ratio:.1f}%)")
...
def test_river_bridge_dropstructure():
    ...
    print(f"  河宽: {river_upstream.width}m")
    ...
    print(f"  桥宽: {bridge.bridge_width}m")
    ...
    # 桥梁壅水
    # compute_discharge返回: (流量, 壅水高度, 流态)
    Q_bridge, backwater, flow_mode = bridge.compute_discharge(h_up, h_up*0.8, river_upstream.width)
    print(f"  桥梁壅水: {backwater:.2f}m")
    print(f"  桥下流态: {flow_mode}")
    print(f"  桥下水深: {h_up + backwater:.2f}m")
    
    # 跌水消能
    h_down_drop, energy_loss, jump_length = drop.compute_energy_loss(Q_flood, h_up)
    print(f"  跌水消能: {energy_loss:.2f}m")
    print(f"  跌后水深: {h_down_drop:.2f}m")
    print(f"  水跃长度: {jump_length:.2f}m")
    ...
    elevations = [
        100.0,
        100.0 + 2500*river_upstream.slope + backwater,  # 修正：使用backwater而不是h_bridge
...
def run_all_tests():
    """运行所有组合测试"""
    print("\n" + "🎯"*40)
    print("补充缺失的7个组合测试 - 修复版".center(80))
    print("🎯"*40)
    ...
    if passed == total:
        ...
        print(f"⚠️  {total-passed}个测试需要进一步调试".center(80))
    print("🎉"*40)
