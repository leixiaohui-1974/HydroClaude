    ...
    if solver.step_count % 100 == 0:
        ...
        if solver.step_count % 500 == 0:
            print(f"  t={state['t']:6.1f}s, h_gate={h_gate:.4f}m, "
                  f"Q_gate={Q_gate:.2f}m³/s, 质量误差={state['mass_error']:.4f}%")
...
print(f"  闸门处流量: {state['Q'][gate_idx]:.2f}m³/s")
print(f"  平均水深: {np.mean(state['h']):.4f}m (理论: {h_uniform:.4f}m)")
print(f"  平均流量: {np.mean(state['Q']):.2f}m³/s (目标: {Q_target:.2f}m³/s)")
...
print(f"  <h>={np.mean(solver2.h):.4f}m, <Q>={np.mean(solver2.Q):.2f}m³/s")
...
print(f"  闸门流量: {state2['Q'][gate_idx]:.2f}m³/s")
...
ax4.set_xlabel('Time (s)', fontsize=11)
ax4.set_ylabel('Gate Discharge (m³/s)', fontsize=11)
