...
class TestCulvertMultiple:
    ...
    def test_ex4_culverts_distinct_positions(self, ex4_ref):
        """Ex4 多涵洞：各涵洞应映射到不同的 us_xs_index（不冲突）。"""
        data = _build_from_ref(ex4_ref)
        culverts_param = data[12]
        if len(culverts_param) <= 1:
            pytest.skip("Ex4 数据中涵洞数量 ≤ 1，跳过多涵洞冲突测试")

        indices = [cv["us_xs_index"] for cv in culverts_param]
        assert len(set(indices)) == len(indices), (
            f"多涵洞映射到相同断面位置: {indices}"
        )

    def test_ex4_5yr_mae_reasonable(self, ex4_ref):
        """Ex4 5yr 工况：MAE 应小于 0.5m（合理范围）。"""
        data = _build_from_ref(ex4_ref)
        _, _, errs, Q, name = _run_profile(ex4_ref, 0, *data)
        mae = float(np.mean(errs))
        assert mae < 0.5, f"Ex4 5yr MAE={mae:.4f}m 超过合理范围 0.5m"
            ...
            "culverts": [
                {
                    ...
                    "width_m": 1.0,
                    "length_m": 10.0,
                    "us_invert_m": 9.0 / 3.281,  # ~2.743m（匹配床面中间值）
