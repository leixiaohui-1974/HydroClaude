...
### 第一次尝试：简单Interface方法（2025-10-29）❌ **失败**

**实施方案**：
- 在_compute_rhs中添加source_term_method='interface'分支
- 从z_b interface计算底坡源项：`S_bed = -g*h*B*dz/dx`
- 保持摩阻源项点值计算
...
**失败原因**：
1. **方法论错误**：Zhou's SGM需要完整实现（重构η而不是h），不能只改源项
2. **数值不一致**：通量用h重构，源项用z_b计算，两者不协调
3. **理解不足**：简化了复杂算法，导致比不实现更糟

**详细分析**：见`docs/INTERFACE_SOURCE_METHOD_FAILURE_ANALYSIS.md`

**状态**：Interface方法已禁用，保留框架等待正确实现

---

### 第二次尝试：Strang Splitting（2025-10-29）❌ **失败**

**实施方案**：
- 添加source_term_treatment参数（'coupled' 或 'strang_splitting'）
- 实现分步求解：通量(dt/2) → 源项(dt) → 通量(dt/2)
- 目标：解耦通量-源项，减少数值误差

**测试结果**：
```
Coupled RK2：质量误差 61.41%
Strang Splitting：质量误差 64.81% ❌ 恶化3.40%
```

**失败原因**：
1. **方法不适用**：MacDonald问题通量-源项耦合强，强行解耦反而有害
2. **精度降低**：源项步用一阶欧拉，通量步降级为一阶，低于coupled RK2
3. **边界条件过度**：每个子步骤都应用BC（3次），引入质量泄漏
4. **理论与实践差距**：Strang Splitting适用于刚性源项，MacDonald问题不适合

**详细分析**：见`docs/STRANG_SPLITTING_FAILURE_ANALYSIS.md`

**状态**：Strang Splitting已实现，但测试证明无效
...
1. **✅ 提高空间精度**（最优先，最有希望）
   - 测试order=2（二阶MUSCL）vs order=1
   - 预期：质量误差可能降到20-30%
   - 理由：一阶精度的数值耗散是很多问题的根源
