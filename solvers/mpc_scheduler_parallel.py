...
class ParallelMPCScheduler:
    ...
    def optimize_step_parallel_pso(
        ...
        for iteration in range(n_iterations):
            ...
            if self.verbose and iteration % 10 == 0:
                print(f"  PSO {iteration}:  = {global_best_cost:.3e}")
    ...
    def run_closed_loop(
        ...
        if self.verbose:
            ...
            print(f": {self.N_pred} ")
            print(f": {self.N_ctrl} ")
            print(f": {optimization_method}")
            print(f": {self.n_workers}")
            print(f"{'='*60}\n")
        ...
        for step in range(n_steps):
            ...
            if step % feedback_interval == 0:
                ...
                if self.verbose:
                    print(f"[OK] (={opt_info['cost']:.3e}, ={opt_info['computation_time']:.3f}s)")
        ...
        if self.verbose:
            print(f"\n{'='*60}")
            print(f"MPC")
            print(f": {total_opt_time:.2f} s")
            print(f": {total_opt_time/(n_steps/feedback_interval):.3f} s/")
