...
from .numerical_methods.rk_solver import RKSolver
...
class PipeHighFidelity:
    ...
    def __init__(self, name: str, length: float, diameter: float,
        ...
        # 初始化
        self.H = np.ones(n_sections) * 50.0  # 水头, 初始设为50m
        ...
        if method in ['rk4', 'rk2']:
            ...
            # Default to RK4 if method is not supported
            print(f"Warning: Method '{method}' not supported for Pipe. Defaulting to 'rk4'.")
            self.solver = RKSolver(method='rk4')
