...
class MOCSolver:
    """特征线法（Method of Characteristics）求解器"""

    @staticmethod
    def solve_canal_internal_boundary(h_left: float, Q_left: float,
    ...
    @staticmethod
    def solve_pipe_internal_boundary(H_left: float, Q_left: float,
                                    H_right: float, Q_right: float,
                                    boundary: BoundaryCondition,
                                    wave_speed: float, area: float) -> Tuple[float, float]:
        """
        求解管道内边界
        Returns: (H_boundary, Q_boundary)
        """
