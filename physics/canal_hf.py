...
from .numerical_methods.preissmann_solver import PreissmannSolver
from .numerical_methods.fvm_solver import FVMSolver
...
class CanalHighFidelity:
    ...
    def step(self, dt: float, boundary_conditions: dict = None) -> dict:
        ...
        if self.method == 'preissmann':
            # Preissmann solver needs special handling for boundary conditions
            h_old = self.h.copy()
            q_old = self.Q.copy()

            # Apply boundary conditions before solving
            if 'upstream_level' in bc:
                h_old[0] = bc['upstream_level']
            if 'downstream_flow' in bc:
                q_old[-1] = bc['downstream_flow']

            self.h, self.Q = self.solver.solve_canal_step(
                h_old, q_old, dt, self.dx,
